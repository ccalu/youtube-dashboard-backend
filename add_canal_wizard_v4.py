# -*- coding: utf-8 -*-
"""
Wizard V4: Adicionar Canal com Upload Automatico
TESTE: Ajustes OAuth para evitar "We couldn't verify it's you"
Mudancas vs V3:
  - Pede email da conta Google (login_hint)
  - Adiciona include_granted_scopes=true
  - Todos os 5 scopes mantidos (upload funciona igual)
"""
...
def print_header():
    ...
    print(f"\n{Fore.CYAN}{'=' * 70}")
    print(f"{Fore.CYAN}|{' ' * 10}{Fore.YELLOW}{Style.BRIGHT}WIZARD V4 - ADICIONAR CANAL (TESTE OAUTH FIX){Style.RESET_ALL}{' ' * 10}{Fore.CYAN}|")
...
def gerar_url_oauth(channel_id, client_id, login_hint=None, is_brand_account=False):
    """Gera URL de autorizacao OAuth

    V4: Adiciona login_hint e include_granted_scopes para evitar
    erro "We couldn't verify it's you" do Google.
    Para Brand Accounts: usa prompt=select_account consent para
    forcar a selecao da conta correta.
    """
    scopes = [
    ...
    # Brand Account: select_account consent (forca escolha da conta)
    # Conta pessoal: consent (direto)
    if is_brand_account:
    ...
    # login_hint ajuda Google a verificar identidade
    # evita "We couldn't verify it's you"
    if login_hint:
...
def main():
    ...
    print(f"  {Fore.WHITE}- {Fore.GREEN}V4: Correcao para erro 'We couldn't verify it's you'{Style.RESET_ALL}\n")
    ...
    # ============================================================
    # PARTE 3: AUTORIZACAO OAUTH (V4 - COM LOGIN HINT)
    # ============================================================
    ...
    # V4: Pede email da conta Google para login_hint
    print(f"  {Fore.CYAN}{Style.BRIGHT}[V4] EMAIL DA CONTA GOOGLE{Style.RESET_ALL}")
    print(f"  {Fore.WHITE}Informar o email ajuda o Google a verificar a identidade")
    print(f"  {Fore.WHITE}e evita o erro 'We couldn't verify it's you'")
    login_hint = prompt("Email da conta Google do canal (ou Enter para pular)")

    if login_hint:
        print_ok(f"Login hint configurado: {login_hint}")
    else:
        print_warning("Sem login hint - se der erro de verificacao, tente novamente com o email")

    # V4: Pergunta se eh Brand Account
    print(f"\n  {Fore.CYAN}{Style.BRIGHT}[V4] TIPO DE CONTA{Style.RESET_ALL}")
    print(f"  {Fore.WHITE}Brand Account = conta do canal (ex: 'Reis Perversos')")
    print(f"  {Fore.WHITE}Conta pessoal = email Gmail direto")
    while True:
        is_brand = prompt("Este canal usa Brand Account? (s/n)").lower()
        if is_brand not in ['s', 'n']:
            print_error("Digite 's' para sim ou 'n' para nao")
            continue
        break
    ...
    if is_brand_account:
        print_ok("Brand Account - URL vai forcar selecao de conta")
    else:
        print_ok("Conta pessoal")

    # Gera URL OAuth (V4: com login_hint + suporte brand account)
    oauth_url = gerar_url_oauth(
        channel_id, client_id,
        login_hint=login_hint if login_hint else None,
        is_brand_account=is_brand_account
    )

    if is_brand_account:
        print(f"\n  {Fore.MAGENTA}{Style.BRIGHT}BRAND ACCOUNT - ATENCAO:{Style.RESET_ALL}")
        print(f"  {Fore.WHITE}  1. Vai aparecer uma lista de contas")
        print(f"  {Fore.WHITE}  2. ESCOLHA A BRAND ACCOUNT (nome do canal), NAO o Gmail!")
        print(f"  {Fore.WHITE}  3. Depois autorize normalmente\n")

    print(f"\n  {Fore.CYAN}{Style.BRIGHT}URL de autorizacao:{Style.RESET_ALL}")
