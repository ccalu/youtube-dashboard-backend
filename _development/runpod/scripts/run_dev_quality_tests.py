#!/usr/bin/env python3
"""LTX 2.3 DEV Model - Quality Tests on RTX 5090
   30 steps, cfg=3.5, dpmpp_2m, strength=0.75, 161 frames (~6.4s)
"""
...
def wf(img, prompt, steps, w, h, name, num_frames=161):
    neg = "blurry, low quality, distorted, watermark, text, flickering, morphing, warping, shaky, cartoon, extra limbs, deformed, compression artifacts"
    return {
        ...
        "8": {"class_type": "VAEDecodeTiled", "inputs": {
            ...
            "temporal_size": 256, "temporal_overlap": 32
        }},
...
tests = [
    ("img1.png", prompt_img1, 30, 720, 1280, "dev_img1_30steps_720p"),
    ("img2.png", prompt_img2, 30, 720, 1280, "dev_img2_30steps_720p"),
]

print("=" * 60)
print("LTX 2.3 DEV — QUALITY TESTS on RTX 5090")
print("Model: DEV FP8 (NOT distilled)")
print("Config: 30 steps | cfg=3.5 | dpmpp_2m | strength=0.75")
print("Output: 720x1280 | 161 frames (~6.4s) | 25fps")
print("=" * 60)
...
for i, (img, p, steps, w, h, name) in enumerate(tests):
    ...
    pid = r["prompt_id"]
    print(f"  Queued: {pid}")
    print(f"  Generating (30 steps DEV, may take 3-10 min)...", end="", flush=True)
    c = wait(pid, 1200)
    ...
    if c and c["status"]["status_str"] == "success":
        for nid, o in c.get("outputs", {}).items():
            for k in ["gifs", "videos"]:
                if k in o:
                    for f in o[k]:
                        print(f"\n  DONE! {elapsed:.0f}s ({elapsed/60:.1f}min) >> {f['filename']}")
...
print("\n" + "=" * 60)
print("RESULTS — LTX 2.3 DEV on RTX 5090")
print("=" * 60)
