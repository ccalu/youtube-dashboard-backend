...
async def check_collection_errors():
    ...
    if not coleta_14_erros:
        print("⚠️  Nenhuma coleta com exatamente 14 erros encontrada")
        ...
        if erros > 0:
            print(f"\n⚠️  Coleta com mais erros: {erros} erros")
            coleta_14_erros = coleta_mais_erros
        else:
            return

    print("✅ Coleta com erros encontrada:")
    ...
    canais_response = db.supabase.table("canais_monitorados")\
        .select("id, nome_canal, url, status")\
        .eq("status", "ativo")\
        .execute()
    ...
    videos_hoje = db.supabase.table("videos_minerados")\
        .select("canal_id")\
