...
<body>
    ...
    <div class="stats">
        ...
        <div class="card">
            ...
            <div class="card-label">Uploads com Sucesso</div>
        </div>
        <div class="card">
        ...
        <div class="card">
            <div class="card-value pending" id="pendente">-</div>
    ...
    <script>
        function forcarUpload(channelId, channelName) {
            if (confirm('Forçar upload do canal ' + channelName + '?')) {
                alert('Upload iniciado para: ' + channelName);
                // Aqui você pode adicionar a chamada API real
            }
        }
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        ...
                        document.getElementById('sem_video').innerText = data.stats.sem_video || 0;
                        document.getElementById('pendente').innerText = data.stats.pendente || 0;
                        ...
                        if (data.subnichos) {
                            for (var subnicho in data.subnichos) {
                                ...
                                for (var j = 0; j < canais.length; j++) {
                                    ...
                                    html += '<td>';
                                    html += '<button class="btn-action btn-upload">📤</button>';
