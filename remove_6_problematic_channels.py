...
# -*- coding: utf-8 -*-
"""
Script para remover 6 canais problemáticos e todos os seus dados.

Canais a remover (conforme solicitado):
#3, #4, #7, #8, #13, #14 da lista de problemas
...
class ProblematicChannelRemover:
    """Classe para remover canais problemáticos com segurança."""
    ...
    async def find_channel_by_url(self, channel_url: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            print(f"[ERRO] Ao buscar canal '{channel_url}': {e}")
            return None
    ...
    def make_backup(self, channels: List[Dict]) -> str:
        ...
        filename = f"backup_canais_problematicos_{timestamp}.json"
        filepath = os.path.join(os.path.dirname(__file__), filename)

        backup_data = {
            'timestamp': timestamp,
            'reason': 'Canais com múltiplas falhas de coleta',
            'channels': channels,
    ...
    async def process_channels(self, channel_data: List[Dict]) -> None:
        """
        Processar remoção de múltiplos canais.

        Args:
            channel_data: Lista de dicionários com URL e nome dos canais
        """
        print("=" * 60)
        print("REMOCAO DE 6 CANAIS PROBLEMATICOS")
        print("=" * 60)
        print()
        ...
        for item in channel_data:
            ...
            channel = await self.find_channel_by_url(url)
            ...
            if channel:
                ...
                print(f"      - Falhas: {channel.get('coleta_falhas_consecutivas', 0)}")
        ...
        # Auto-confirmar com --confirm
        confirm = 'DELETAR'
        print("   Confirmacao automatica ativada")
...
async def main():
    """Função principal."""
    # Os 6 canais problemáticos para remover (#3, #4, #7, #8, #13, #14)
    PROBLEMATIC_CHANNELS = [
        {'url': 'https://www.youtube.com/@WW2NavalHistory', 'name': 'WW2 Naval History'},
        {'url': 'https://www.youtube.com/@CoronadoSecreto', 'name': 'Secretos de los Coronados'},
        {'url': 'https://www.youtube.com/@ChroniquesDeLaSecondeGuerre', 'name': 'Chroniques de la Seconde Guerre'},
        {'url': 'https://www.youtube.com/@TracesofVoice', 'name': 'Traces of Voice'},
        {'url': 'https://www.youtube.com/@WarEchoes35', 'name': 'WAR ECHOES'},
        {'url': 'https://www.youtube.com/@Historia_EnSecreto', 'name': 'Historia En Secreto'}
    ]

    remover = ProblematicChannelRemover()

    try:
        await remover.process_channels(PROBLEMATIC_CHANNELS)
