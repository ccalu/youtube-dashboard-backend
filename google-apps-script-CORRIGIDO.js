 ...
 * Monitora aba "Página1" e envia webhook quando vídeo está pronto para upload.
 */
function onEdit(e) {
  const sheet = e.source.getActiveSheet();

  // Só monitora aba "Página1" (SEM ESPAÇO)
  if (sheet.getName() !== 'Página1') {
    Logger.log('⏭️ Ignorando edição (não é aba Página1)');
    return;
  }
...
5. File → Save (URL do Railway já está configurada!)
6. Trigger já deve estar configurado (não precisa recriar)
...
✅ APÓS WEBHOOK ENVIADO:
- Coluna O marcada como "✅ done" imediatamente
- Backend faz upload
- Se der erro, backend sobrescreve com "❌ Erro"
