#!/bin/bash
# ========================================
# SYNC.SH - Sincronizacao Universal v2.1
# Dashboard de Mineracao YouTube
# ========================================
# Com logs detalhados e verificacao de erros
# Suporta mensagem personalizada: ./sync.sh "minha mensagem"
# ========================================

# Capturar mensagem personalizada (se fornecida)
CUSTOM_MSG="$1"
...
if [ -n "$CUSTOM_MSG" ]; then
    echo -e "${CYAN}Mensagem:${NC} ${YELLOW}$CUSTOM_MSG${NC}"
fi
echo ""
echo -e "${YELLOW}────────────────────────────────────────${NC}"
...
if ! git diff-index --quiet HEAD -- 2>/dev/null; then
    # Usar mensagem personalizada se fornecida, senao usar padrao
    if [ -n "$CUSTOM_MSG" ]; then
