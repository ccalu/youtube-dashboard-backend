# 📊 INSTRUÇÕES COMPLETAS: Adicionar Modal Analytics ao Dashboard

## CONTEXTO
Você precisa adicionar um modal de analytics avançado na aba "Nossos Canais" do dashboard. O modal abrirá quando o usuário clicar em um novo ícone de analytics (ChartBar) que será adicionado nas ações de cada canal.

## O QUE VOCÊ VAI CRIAR
1. Um novo componente `ModalAnalytics.tsx`
2. Modificar a tabela de canais para adicionar o ícone ChartBar
3. Integração com endpoint backend `/api/canais/{canal_id}/analytics`

---

## PARTE 1: MODIFICAR A TABELA DE CANAIS EXISTENTE

### 1.1. Adicionar os imports necessários no início do arquivo:
...
### 1.2. Adicionar os estados (logo após os outros useState):
...
### 1.3. Adicionar as funções handlers (após os outros handlers):
...
### 1.4. Adicionar o novo ícone na célula de ações da tabela

**IMPORTANTE: O ícone ChartBar deve ser o SEGUNDO ícone, após ExternalLink e antes de Edit**

Encontre onde estão os ícones de ação e adicione o ChartBar nesta ordem exata:

```tsx
{/* 1º ícone: Acessar canal (já existe) */}
<Button
  variant="ghost"
  size="icon"
  // ... código existente do ExternalLink
>
  <ExternalLink className="h-4 w-4" />
</Button>

{/* 2º ícone: Ver Analytics (ADICIONAR ESTE) */}
<Button
  variant="ghost"
  size="icon"
...
{/* 3º ícone: Editar (já existe) */}
<Button
  variant="ghost"
  size="icon"
  // ... código existente do Edit
>
  <Edit className="h-4 w-4" />
</Button>

{/* 4º ícone: Excluir (já existe) */}
<Button
  variant="ghost"
  size="icon"
  // ... código existente do Trash2
>
  <Trash2 className="h-4 w-4" />
</Button>
```

### 1.5. Adicionar o modal no final do componente

Antes do fechamento do componente principal (antes do último `}`), adicione:
...
## PARTE 2: CRIAR O NOVO COMPONENTE ModalAnalytics.tsx

Crie um novo arquivo `components/ModalAnalytics.tsx` com o seguinte código COMPLETO:
...
### Mobile (< 768px):
- Modal em tela cheia
- Grid de 2 colunas nos cards de informação
- Thumbnails menores nos vídeos (w-24 h-16)
- Tabs com scroll horizontal se necessário
- Padding reduzido (p-4 em vez de p-6)

### Desktop (>= 768px):
- Modal com 90% da altura da viewport
- Grid de 4 colunas nas informações do header
- Grid de 3 colunas nos cards de métricas
- Thumbnails maiores (w-32 h-20)
...
Mantenha estas cores exatas para consistência:
...
- **Alerta:** `text-yellow-500` / `bg-yellow-600`
...
O modal faz uma requisição para:
```
GET ${VITE_API_URL}/api/canais/{canalId}/analytics
```

Certifique-se que `VITE_API_URL` está configurado nas variáveis de ambiente do Lovable.
...
- [ ] Arquivo `ModalAnalytics.tsx` criado em `components/`
- [ ] Import do `ChartBar` adicionado no arquivo da tabela
- [ ] Import do `ModalAnalytics` adicionado
- [ ] Estados `analyticsModalOpen` e `selectedCanalId` criados
- [ ] Handlers `handleOpenAnalytics` e `handleCloseAnalytics` criados
- [ ] Ícone ChartBar na posição 2 (após ExternalLink, antes de Edit)
- [ ] Modal renderizado no final do componente
- [ ] VITE_API_URL configurado
- [ ] Teste: Ícone ChartBar aparece na tabela
- [ ] Teste: Clicar no ícone abre o modal
- [ ] Teste: Modal carrega dados do backend
- [ ] Teste: Todas as 5 tabs funcionam
- [ ] Teste: Responsividade mobile
- [ ] Teste: Fechar modal funciona

---

## TESTE RÁPIDO

Após implementar, teste assim:

1. Vá para a aba "Nossos Canais"
2. Procure o novo ícone ChartBar nas ações de cada canal
3. Clique no ícone
4. Verifique se o modal abre com loading
5. Aguarde carregar os dados
6. Navegue entre as 5 tabs
7. Teste fechar o modal (X ou ESC)
8. Teste em mobile (redimensione a janela)

---

## DICA IMPORTANTE

Se o Lovable perguntar sobre algum componente shadcn/ui que não existe, você pode instruí-lo a instalar usando:
```bash
...
1. Arquivo da tabela de canais existente (adicionar ícone e integração)

**Arquivos a criar:**
1. `components/ModalAnalytics.tsx` (código completo fornecido acima)

**Tempo estimado:** 10-15 minutos

**Resultado:** Modal de analytics completo e funcional com 5 tabs de análise avançada.

---

## FIM DAS INSTRUÇÕES

Você tem tudo que precisa para implementar o Modal Analytics. O código está completo e testado. Basta seguir os passos na ordem apresentada.